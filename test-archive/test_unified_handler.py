...
"""
    ...
    if mock_llm.integrate_answers.called:
        ...
        if (
            call_args.kwargs.get("llm_profile") == handler_config.llm_profile
            and call_args.kwargs.get("output_format") == handler_config.output_format
        ):
...
"""
    ...
    result_with_registry = runner_with_registry._execute_section(
        "requirements", state, dry_run=True
    )
    print(f"  With registry: action={result_with_registry.action_taken}")

    # Execute without registry (should use legacy phase dispatch)
    result_without_registry = runner_without_registry._execute_section(
        "requirements", state, dry_run=True
    )
