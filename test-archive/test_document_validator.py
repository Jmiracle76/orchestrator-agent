...
def create_test_document(
    ...
    if has_review_gate_result:
        ...
        lines.append(
            f"<!-- review_gate_result:review_gate:test_gate status={status} issues={issues} warnings=0 -->"
        )
        lines.append("")
...
def test_strict_mode_with_deferred_questions():
    ...
    if status_normal.complete and not status_strict.complete:
        ...
        print(
            f"  ✗ Normal mode complete: {status_normal.complete}, Strict mode complete: {status_strict.complete}"
        )
        return False
...
def test_human_readable_output():
    ...
    if status.summary:
        ...
        if has_status and has_icons and has_blocking:
            ...
            print(
                f"  ✗ Summary missing elements: status={has_status}, icons={has_icons}, blocking={has_blocking}"
            )
            return False
    else:
