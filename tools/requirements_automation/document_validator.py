...
class DocumentValidator:
    ...
    def _get_required_sections(self) -> List[str]:
        ...
        """
        return [t for t in self.workflow_order if not is_special_workflow_target(t)]
    ...
    def _section_complete(self, section_id: str) -> bool:
        ...
        for q in open_qs:
            if (
                q.status.strip() == QUESTION_STATUS_OPEN
                and _canon_target(q.section_target) in target_ids
            ):
                return False
    ...
    def _build_summary(
        ...
        if not complete:
            lines.append("")
            lines.append("Document cannot proceed to downstream automation until all criteria met.")
