...
from .models import SectionState, SectionSpan, SubsectionSpan
from .config import PLACEHOLDER_TOKEN, TARGET_CANONICAL_MAP
from .parsing import find_sections, get_section_span, section_is_locked, section_is_blank, find_subsections_within, section_body
...
def get_section_state(lines: List[str], target_id: str) -> SectionState:
    """
    ...
    has_placeholder = any(
        ...
        for line in lines[span.start_line : span.end_line]
    )
...
def gather_prior_sections(lines: List[str], workflow_order: List[str], target_id: str) -> dict[str, str]:
    """
