...
"""
    ...
    if open_q_sub:
        print(f"  Open questions subsection: start={open_q_sub.start_line}, end={open_q_sub.end_line}")
...
"""
    ...
    mock_llm = Mock()
    mock_llm.integrate_answers = Mock(return_value="""### Identified Risks
...
2. **Scalability Risk**: Database bottlenecks under load""")
    ...
    print(f"  Section state: is_blank={state.is_blank}, has_answered_questions={state.has_answered_questions}")
...
"""
    ...
    mock_llm = Mock()
    mock_llm.draft_section = Mock(return_value="""### Identified Risks
