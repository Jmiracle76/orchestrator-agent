...
def test_template_creation_commits_file():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        subprocess.run(["git", "remote", "add", "origin", str(bare_repo)], cwd=test_repo, check=True, capture_output=True)
        ...
        subprocess.run(["git", "commit", "-m", "Initial commit with template"], cwd=test_repo, check=True, capture_output=True)
        subprocess.run(["git", "push", "-u", "origin", "master"], cwd=test_repo, check=True, capture_output=True)
