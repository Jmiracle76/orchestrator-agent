...
def test_ac4_llm_returns_structured_json():
    ...
    for field in required_fields:
        if not hasattr(ReviewResult, '__dataclass_fields__') or \
           field not in ReviewResult.__dataclass_fields__:
            print(f"  ✗ ReviewResult missing required field: {field}")
            return False
    
    # Verify ReviewIssue has severity and description
    if not hasattr(ReviewIssue, '__dataclass_fields__') or \
       'severity' not in ReviewIssue.__dataclass_fields__ or \
       'description' not in ReviewIssue.__dataclass_fields__:
...
def test_ac5_patch_validation():
    ...
    result = ReviewResult(
        ...
        patches=[
            ...
            ReviewPatch(section="test", suggestion="<!-- invalid -->", rationale="test", validated=False),
        ],
        scope_sections=["test"],
        summary="test",
    )
...
def test_ac7_auto_apply_configurable():
    ...
    result = ReviewResult(
        ...
        issues=[],
        patches=[
            ReviewPatch(section="test", suggestion="new", rationale="test", validated=True)
        ],
        scope_sections=["test"],
        summary="test",
    )
    ...
    config_never = HandlerConfig(
        section_id="test", mode="review_gate", output_format="prose",
        subsections=False, dedupe=False, preserve_headers=[], sanitize_remove=[],
        llm_profile="requirements_review", auto_apply_patches="never",
        scope="entire_document", validation_rules=[],
    )
    ...
    config_if_valid = HandlerConfig(
        section_id="test", mode="review_gate", output_format="prose",
        subsections=False, dedupe=False, preserve_headers=[], sanitize_remove=[],
        llm_profile="requirements_review", auto_apply_patches="if_validation_passes",
        scope="entire_document", validation_rules=[],
    )
...
def test_ac8_scope_configurable():
    ...
    if scope2 != ["section1", "section2", "section3"]:
        print(f"  ✗ entire_document returned {scope2}, expected ['section1', 'section2', 'section3']")
        return False
